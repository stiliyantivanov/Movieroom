<x-home>
    ...
    @if(!$series->isEmpty())
        <div class="flex items-center justify-center mb-6">
            ...
            <a href="{{route('series',['sort'=>'name'])}}" class="bg-gray-500 hover:bg-gray-600 text-white text-xl font-bold px-6 py-1 rounded-l-lg">
                Name
            </a>
            <a href="{{route('series',['sort'=>'year_of_release'])}}" class="bg-gray-500 hover:bg-gray-600 text-white text-xl font-bold px-6 py-1">
                Year of release
            </a>
            <a href="{{route('series',['sort'=>'reviews'])}}" class="bg-gray-500 hover:bg-gray-600 text-white text-xl font-bold px-6 py-1 rounded-r-lg">
                No. of reviews
            </a>
        </div>
        @foreach($series as $movie)
            <div class="mb-6">
                ...
                <div class="flex items-center justify-end">
                    ...
                    @endauth  
                    @can('update', $movie) 
                    <a href="/series/{{$movie->id}}/edit" class="rounded-lg bg-green-400 hover:bg-green-600 shadow-lg text-xl text-white font-bold mb-6 mr-2 px-6 py-1">
                        Update
                    </a>
                    @endcan
                    @can('delete', $movie)
                    <form method="post" action="/series/{{$movie->id}}/destroy">
                        @csrf
                        @method('DELETE')
        ...
        @endforeach
    @else
        <p class="text-xl text-white">There are no series yet.</p>
    @endif
</x-home>